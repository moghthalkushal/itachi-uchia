...
/* Fills the canvas with 4K high quality image smoothing */
...
function unlockAudio() {
  ...
  if (bgmAudio) {
    ...
    if (bgmOn && bgmAudio.paused) {
      bgmAudio.play().then(() => {
        if (bgmToggle) bgmToggle.setAttribute('aria-pressed', 'true');
        if (bgmState) bgmState.textContent = 'ON';
      }).catch(err => {
        console.warn('BGM unlock error:', err);
      });
    }
  }
}
...
function pauseBGM() {
  if (!bgmAudio) return;
  bgmOn = false;
  if (bgmToggle) bgmToggle.setAttribute('aria-pressed', 'false');
  if (bgmState) bgmState.textContent = 'OFF';
  bgmAudio.pause();
}
